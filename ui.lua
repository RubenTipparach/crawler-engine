...
function UI.fog_tweak_draw()
	...
	local x, y = 2, 26
	local n = #fog.colors
	local labels = {"color", "depth", "dither"}
	local mode = labels[UI.fog_mode]

	rectfill(x-1, y-1, x+180, y + n * 10 + 28, 0)
	print("fog ["..mode.."] i/k:slot j/l:val", x, y, 6)
	y += 10

	for i = 1, n do
		local sel = i == UI.fog_slot and UI.fog_mode ~= 3
